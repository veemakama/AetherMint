...
router.post('/optimize',
  ...
  [
    ...
    body('contentType')
      ...
      .withMessage('Content type must be one of: video, image, audio, document, other'),
    ...
    body('requestedQuality')
      ...
      .withMessage('Requested quality must be one of: auto, 360p, 720p, 1080p, 4k'),
    
    body('optimizationLevel')
      .optional()
      .isIn(['basic', 'standard', 'aggressive'])
      .withMessage('Optimization level must be one of: basic, standard, aggressive'),
...
router.put('/configuration',
  [
    ...
    body('defaultQuality')
      .optional()
      .isIn(['auto', '360p', '720p', '1080p', '4k'])
      .withMessage('Default quality must be one of: auto, 360p, 720p, 1080p, 4k'),
    ...
    body('maxConcurrentOptimizations')
      .optional()
      .isInt({ min: 1, max: 1000 })
      .withMessage('Max concurrent optimizations must be an integer between 1 and 1000'),
    
    body('optimizationTimeout')
      .optional()
      .isInt({ min: 5, max: 300 })
      .withMessage('Optimization timeout must be an integer between 5 and 300 seconds')
  ],
  validateRequest,
 ...
 */
router.delete('/optimizations/:requestId',
  [
    param('requestId')
      .notEmpty()
 ...
 */
router.get('/optimizations/active',
...
router.delete('/history',
  controller.clearHistory.bind(controller)
);
