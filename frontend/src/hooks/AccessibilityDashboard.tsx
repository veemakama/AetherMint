...
export const AccessibilityDashboard: React.FC = () => {
  ...
  useEffect(() => {
    return () => {
      if (auditTimerRef.current !== null) {
        window.clearTimeout(auditTimerRef.current);
      }
    };
  }, []);
  ...
  return (
    <section className="p-6 max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-md" aria-labelledby="accessibility-dashboard-title" aria-busy={isAuditing}>
      ...
      <div className="space-y-4" aria-live="polite" aria-atomic="true">
        ...
        {results.length > 0 && (
          <ul className="space-y-4" role="list" aria-label="Accessibility audit results">
            {results.map(result => (
              <li
            key={result.id} 
