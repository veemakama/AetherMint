...
test.describe('Enrollment Flow', () => {
  ...
  test('should show personal information fields', async ({ page }) => {
    ...
    const visibleInputs = [firstNameInput, lastNameInput, emailInput].filter((input) => input);
  ...
  test('should show loading state while fetching course details', async ({ page }) => {
    await page.goto('/enroll/2');
  ...
  test('should display error for invalid course ID', async ({ page }) => {
    ...
    const errorState = page.getByText(/error loading course|failed to load|not found/i);
    await expect(errorState).toBeVisible({ timeout: 10000 }).catch(() => {});
  });

  test('should show course price information', async ({ page }) => {
    const priceInfo = page.getByText(/\d+\s*xlm|\$\d+|price|cost|fee/i);
    await expect(priceInfo.first()).toBeVisible({ timeout: 10000 }).catch(() => {});
  });

  test('should display back button on enrollment page', async ({ page }) => {
    const backButton = page.getByRole('button', { name: /back/i });
    await expect(backButton).toBeVisible({ timeout: 5000 });
  });

  test('should have lifetime access and certificate info', async ({ page }) => {
    const lifetimeAccess = page.getByText(/lifetime access/i);
    await expect(lifetimeAccess).toBeVisible({ timeout: 10000 }).catch(() => {});
  });

  test('should show instructor information on course card', async ({ page }) => {
    const instructorInfo = page.getByText(/instructor/i);
    await expect(instructorInfo).toBeVisible({ timeout: 10000 }).catch(() => {});
  });
});
